...
# Usage: ./scripts/certbot-init.sh
#
# Requirements:
#   - DNS A records for SERVER_NAME and meet.SERVER_NAME point to EXTERNAL_IP
#   - Router forwards WAN 80 -> host 60080 (nginx HTTP)
#   - nginx must serve /.well-known/acme-challenge from DATA_DIR/nginx/html
...
echo "Cert storage:  ${CERTS}"
echo ""
echo "Make sure nginx is running and reachable on WAN port 80."
echo ""

# Run certbot in a one-shot container
docker run --rm \
...
echo ""
echo "Reloading nginx and restarting coturn/synapse to pick up new cert..."
